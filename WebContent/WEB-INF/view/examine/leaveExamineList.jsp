...
<div class="layui-layout layui-layout-admin">
  ...
  <div class="layui-body">
    ...
    <div style="padding: 20px;">
    	<form action="findQjExamineMore.do" method="post" id="form" class="layui-form layui-form-pane1">
	    <div >
		  <c:choose>
			    	...
			    	<tr>
			    		...
			    		<td>
			    			<c:if test="${fn:substring(userExist.zwdm.zwdm,fn:length(userExist.zwdm.zwdm)-2,fn:length(userExist.zwdm.zwdm)) eq '01' }">
			    				<c:choose>
			    				<c:when test="${userExist.yhzh ne 'admin' }">
			    					<c:if test="${userExist.bmdm.bmdm eq leave.bmdm.bmdm }">
			    						<c:if test="${userExist.yhdm ne leave.qjr.yhdm }">
			    						<c:if test="${leave.wczt.ztms eq '未审核' }">
			    							<a class="edit" href="toLeaveExamine.do?qjid=${leave.qjid }">审核 </a>
			    						</c:if>
			    						</c:if>
			    					</c:if>
			    				</c:when>
			    				<c:otherwise>
			    					<c:if test="${leave.wczt.ztms eq '待上级审核' }"> <!--是管理员  判断部门组长是否审核 -->
			    						<a class="edit" href="toLeaveExamine.do?qjid=${leave.qjid }">审核 </a>
			    					</c:if>
			    					<c:if test="${fn:substring(leave.qjr.zwdm.zwdm,fn:length(leave.qjr.zwdm.zwdm)-2,fn:length(leave.qjr.zwdm.zwdm)) eq '01' }">
			    						<c:if test="${leave.wczt.ztms eq '未审核' }">
			    							<a class="edit" href="toLeaveExamine.do?qjid=${leave.qjid }">审核 </a>
			    						</c:if>
			    					</c:if>
