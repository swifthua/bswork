...
<head>
  ...
  <script>
	...
	function deleteByRzid(rzid){
		 layer.confirm('真的删除行么', function(){
	        window.location.href="delByRzid.do?rzid="+rzid;
	     }); 
	}
...
<div class="layui-layout layui-layout-admin">
  ...
  <div class="layui-body">
    ...
    <div style="padding-left: 20px;padding-top: 10px;">	当前位置:<span style="color: blue;">考勤管理/出勤绩效查询</span></div>
    <div style="padding: 20px;">
    	<form action="findRzMore.do" method="post" id="form" class="layui-form layui-form-pane1">
	    <div >
		  <c:choose>
		...
		<br>
			<table id="table" class="layui-table">
			  ...
			  <tbody>
			    <c:forEach items="${pb.list }" var="journal">
			    	<tr>
			    		<td>${journal.djr.yhxm}</td>
			    		<td>${fn:substring(journal.rzsj,0,16)}</td>
			    		<td>${journal.rznr }</td>
			    		<td>8</td>
			    		<td>${journal.duration }</td>
			    		<td>${fn:substring(journal.djsj,0,16)}</td>
			    		<td>
			    			<a class="view" href="viewByRzid.do?rzid=${journal.rzid }">查看</a>  
			    			<c:if test="${userExist.yhzh eq 'admin' }">
			    				<a class="del"  onclick="deleteByRzid('${journal.rzid }')" layEvent="del">删除</a>
			    			</c:if>
			    		</td>
			    	</tr>
			    </c:forEach>
